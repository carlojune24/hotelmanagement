...
- Guest arrives at `/{slug}` (redirected there automatically for non-staff visitors), searches by date range + occupancy, sees available room types with rate-plan price breakdowns (`lib/server/availability.ts`, `lib/server/pricing.ts` already compute this server-side), fills guest details, and pays.

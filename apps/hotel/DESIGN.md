<!-- AS-BUILT: the guest booking flow (search, results, details, review, confirmation) is implemented under src/routes/[hotel]/(guest)/ per this file. Tokens and components below reflect the shipped implementation, not a seed. -->
...
**Scope: the public, unauthenticated `/{slug}/…` guest-facing booking surface only** — search, results, guest details, review, PayMongo redirect, confirmation. It is a deliberately separate world from the authenticated staff app (`management` routes), which keeps its existing neutral shadcn/oklch operate-mode theme untouched. A staff route never inherits this file; a guest route never inherits the staff theme.
...
### Neutral
- **Ledger Paper** (`--ledger-paper`, per-hotel, set inline from `branding.paperColor`, default pure white): base surface — every other page/section background, plus the text color for anything filled with the accent (`.ledger-btn-primary`, the confirmation ticket's outer frame). Computed server-side in `(guest)/+layout.server.ts`'s `theme.paper`, same mechanism as `theme.accent`.
...
**Display Font:** per-hotel selectable, Literata Variable (serif, with Georgia fallback) by default. A hotel can instead choose Playfair Display, Fraunces, or Cormorant Garamond — four curated options in `$lib/branding.ts`'s `DISPLAY_FONTS`, each a self-hosted `@fontsource-variable/*` package (no Google Fonts CDN call at request time, consistent with how Literata/Inter/JetBrains Mono already ship). Picked in `management/settings/branding` (a 2×2 grid of live "Aa" previews, one per option), resolved into `--ledger-font-display` inline per request in `(guest)/+layout.svelte`'s `rootStyle` — every `h1`/`h2`/`h3`/`.ledger-display` rule reads that variable rather than a hardcoded font-family, so the choice reaches the whole storefront (hero, room names, room-photo initial mark, confirmation ticket) from one place.
...
`/dates` — wizard step 1, "Check-in & Check-out Date." Plain check-in/check-out date fields (not a custom two-month range-calendar widget — a deliberate scope call this pass, in favor of the same native/shadcn date inputs already proven elsewhere in this world, over building new calendar UI from scratch) plus Adults/Children steppers ("Guests, per room" — the only place occupancy is collected anywhere in the flow) and the accessible-only checkbox (moved here from the old homepage search dock). Reached cold from a Room Detail Page's "Book Now" in undated mode, or from the Booking Summary Sidebar's "Edit" links (pre-filled with whatever's currently known) — never from the homepage's own "Check availability," which already has dates and skips straight to Select Rooms & Rates.

### Static Content Pages (Dining / Meetings & Events / Contact Us)

Three more Persuade-mode pages, same posture as the Room Detail Page (own URL, `StorefrontNav` at top, no step chrome): `/dining`, `/meetings-events`, `/contact`. All three, plus the homepage, now share one **`StorefrontNav`** component (`lib/components/storefront/storefront-nav.svelte`) instead of four copies of the same nav markup — it renders every optional link (Meetings & Events, Dining, Reviews) conditionally on real data, and deliberately uses **absolute `/{hotelSlug}/...` paths throughout**, not this route tree's usual relative-link convention, since the same component now renders from several different nesting depths and a hand-computed `../` chain has been the single most repeated bug this pass (caught five separate times, always with the same fix: verify with Node's `URL` class, never eyeball it). `functionHalls`/`reviews`/`diningItems` (plus the `dining` config) all live in the shared `(guest)/+layout.server.ts` load so the nav has them on every page, not just the homepage.

- **Dining** (`/dining`): a structured multi-venue model, not free text — reversed from an earlier free-text pass once the admin asked for per-venue photo/title/description/hours. Each venue is its own `dining_items` row (a real table, own settings CRUD at `management/settings/dining`, own detail page per item — mirrors the Function Hall table's modeling, since each venue needs its own photo-upload lifecycle). **Restructured (this pass)** from a bare stacked-text page into a real promotional page, after the client's old site's dining section (one dark hero-style block with an intro + bulleted highlights + a photo grid of venues) was cited as more appealing than what shipped first — the *structure* was adopted, re-skinned entirely in this world's own vocabulary, never the dark/gold skin itself:
  - **Intro block** (optional, hotel-admin editable — `hotels.config.dining.introEyebrow`/`introHeading`/`introBody`, same jsonb-sibling-to-`branding` pattern the menu gallery already used): an eyebrow + `ink-heading` + lede, the same device About/Amenities/Gallery use on the homepage. Unset → falls back to the original plain "Dining at {hotel}" title, never an empty eyebrow/heading. A "View menu" button (`.ledger-btn-primary`, jumps to `#menus`) appears only when the hotel actually has menu photos uploaded.
  - **Teaser grid** (`.storefront-room-grid`/`.storefront-room-card` — the exact same photo-card component the homepage's Rooms & Rates browse grid uses, not a new one): one card per venue, cover photo (or the woven-pattern + initial-letter mark fallback `.ledger-room-photo-mark` already used for a room/hall with no photo), name overlaid in Literata, the venue's `category` tag (free text, e.g. "Restaurant" — no enum, categories vary too widely hotel to hotel) on the left of the overlay's bottom row, "View →" on the right. Each card is a same-page anchor link (`#venue-{id}`) down to that venue's full section below, not a route change — a single venue still renders as one card, not a broken grid.
  - **Full venue section** (`id="venue-{id}"`, `scroll-mt-24` — same anchor-jump convention Meetings & Events' hall sections use): category label, Literata title, optional `tagline` (one-line pitch, shown directly under the title), a real photo gallery reusing the Room Detail Page's exact `.storefront-room-dialog-photo-main`/`-photo-strip`/lightbox idiom (one `cover`-tagged shot plus any `gallery`-tagged ones, same `DiningPhoto[]` cover/gallery convention `room_types.photos` established), description, up to 6 `highlights` (short selling points) rendered as a checklist reusing `.storefront-amenity-list`/`.storefront-amenity-item` + `CircleCheckIcon` (the same device the Room Detail Page's own amenity list uses), and the operating-hours spec chip (now with a `ClockIcon`). Every field is independently conditional — a venue with only a title and a photo still renders cleanly, per this world's "graceful with partial data" principle.
  - Below the venue list, unchanged: a separate **menu photo gallery** (`hotels.config.dining.menuImages`) — same `.storefront-gallery-grid` + lightbox as the main Gallery section, now anchored `id="menus"` so the intro's "View menu" button has somewhere to jump.
  - Omits itself gracefully ("Dining information is coming soon") if there are no venues yet; the nav link itself only appears once there's a venue or a menu photo to show.
- **Meetings & Events** (`/meetings-events`): a fuller presentation of the **same real Function Hall data** the homepage's `#function-hall` section already uses (`listFunctionHalls`) — full photo gallery per hall (reusing the Room Detail Page's `.storefront-room-dialog-photo-main`/`-photo-strip` gallery pattern), full description, capacity as a spec chip, included services and supported event types as their own labeled blocks, and a **"Book this hall"** button linking to `/#function-hall` — the live reservation mini-form and quote endpoint stay exactly where they are on the homepage; this page is informational, not a second booking form. This is a hotel's Room Detail Page equivalent for its function hall(s): each hall's `<section>` carries `id="hall-{hallId}"` (`scroll-mt-24` so the sticky nav doesn't cover it on jump), and the homepage's own `#function-hall` card links into it via a **"View full details →"** affordance (`.storefront-view-details-btn`, the same class the Rooms & Rates browse grid uses) — no separate per-hall route was built, since this page already shows everything a dedicated one would, just for every hall on one page rather than one hall per URL.
- **Contact Us** (`/contact`): `branding.contactAddress`/`contactPhone`/`contactEmail` as plain ruled rows (`.storefront-contact-row`, an icon + the fact — `tel:`/`mailto:` links, not a form; this app's existing pattern is guests reaching the hotel directly, not through a ticketing system) plus an embedded map (`.storefront-contact-map`), still a **plain Google Maps embed, no API key**. It prefers a precise `contactLat`/`contactLng` pin (`google.com/maps?q=<lat>,<lng>&output=embed`) — dropped by staff on a real interactive map in Branding settings, see below — falling back to the old address-search embed (`q=<address>`) when no pin is set. Every row and the map itself are independently conditional — a hotel that's only set a phone number shows just that row, no empty placeholders for the rest.

Meetings & Events and Contact Us stay hotel-admin-editable from `management/settings/branding` (Check-in/Check-out policy, Contact fields — the same jsonb-backed `branding` object, no new tables, no migration). Dining has its own settings area (`management/settings/dining`, a settings-index tile of its own) — a `dining_items` table (list/create/edit/delete, single-photo replace semantics per venue) plus the separate `hotels.config.dining.menuImages` gallery (multi-photo append semantics, matching the Gallery-photos pattern).
...
## Downpayment (partial online payment)

When a rate plan's cancellation policy asks for only a share up front, the guest surfaces say so in the same two registers as everything else — never a new colour, badge or alert. **Rate plan row:** one mono line, "50% due at booking · balance at the hotel", under the cancellation badge. **Cart / floating invoice:** two ruled rows under Total, "Due now" (ink) and "Due at the hotel" (muted), only when a balance exists — a preview; the server recomputes. **Review page:** the itemised bill is unchanged; after Total come "Due now via PayMongo" (semibold) and "Due at the hotel" (muted), the button reads "Pay ₱X now with PayMongo", and the redirect note adds one sentence about the balance. **Confirmation ticket / manage page / email:** "Total paid" becomes "Paid" plus "Due at the hotel" in the manifest table; a pay-in-full order keeps "Total paid" and reads exactly as before. Source: `lib/downpayment.ts`, `lib/server/order-payment.ts`.

## Staff — booking Transaction (operate mode)

Not part of the Woven Ledger world: it uses the neutral shadcn + oklch staff shell. A multi-room booking is one parent (`orders`) with N rooms, and its money is settled at the parent. **Transaction page** (`/{slug}/management/transactions/[orderId]`): header with guest, booking code and status; left column the rooms/events table (each row links to its reservation) and the payments list (Receipt, Void); right column a Charges / Paid / Balance block whose balance is the one number that matters (red while owed, "Settled" at zero, "Credit" when overpaid) and one **Take payment** for the whole booking. **Walk-in Transaction dialog:** rooms and total, guest, payment, and a live "balance still due" line so a short payment is explicit before confirming. Rooms of one booking never show a per-room share of a payment — reservation and front-desk panels show "This room / Whole booking / Paid / Booking balance" with an "Open transaction" link. Source: `lib/ledger.ts`, `getOrderLedger` in `lib/server/folio.ts`, `lib/server/order-balances.ts`.

**Update — per room:** the Transaction page is a folio card per room (charges, deposit, payments on that room, city-ledger entries, room balance) with one Take payment that splits across rooms; "Whole booking" only sums the rooms. Reservation and front-desk figures are per room again.

## Staff — Dashboard (operate mode)

Not part of the Woven Ledger world: the neutral shadcn + oklch staff shell. The post-login landing page (`/{slug}/management/dashboard`), written first for the front desk on a tablet; every figure links to the screen that owns it and the page does nothing else. **Top to bottom:** header "Today" + the hotel's business date; a **Needs attention** list shown only when non-empty (yesterday not day-closed in `--danger` — only for a hotel that has ever run day close; open cashier shifts, pending damage reports, unread guest messages in ink), no "all clear" filler; the **Rooms board** — one ruled panel split into four hairline cells (`gap-px` over `--border`, never four floating cards): Arrivals, Departures (a `--danger` "N overdue" subline when a checked-in stay is past its checkout date), In-house guests, Occupancy % with a thin `--brand` bar (out-of-order rooms excluded) — 2×2 on a portrait tablet, 4-across at `lg`; the **Money board** (finance:read only) — Cash on hand and Collected this month, labelled "cash basis"; a small booking-site link row. Figures: Inter 600, `tabular-nums`, 2.5rem (rooms) / 1.875rem (money). Sections a role can't see are omitted, never zeroed; a hotel with no rooms sees the two setup cards instead of the Rooms board. **Not here, on purpose:** charts, sparklines, trend deltas, date pickers — Finance and Reports own those. Source: `+page.server.ts` (reuses `getRoomStatusGrid`, `getCashPosition`, `revenueBySourceReport`, `listShifts`, `getDayCloseStatus`), `$lib/room-summary.ts`.

## Staff — Reservations list (operate mode)

Neutral shadcn + oklch staff shell. `/{slug}/management/reservations` is a worklist, not an archive dump: **tabs** matching how staff think — Upcoming (default; not yet checked in, soonest check-in first, a missed arrival stays on top in red), In-house (earliest check-out first, overdue in red), Past (most recent first), Cancelled & no-show, All (newest booked first) — each with its count. Filtering, search (guest, email, booking code, room number) and paging (25) happen on the server, and tab/type/search/page live in the URL so Back and shared links work. Rows tell look-alike bookings apart: booking code (mono) + "booked Sep 23" under the guest, room number(s) before the room type, dates as "Sep 23 → 24" + nights. Status column stacks the badge with any red flag (arrival due, overdue, balance owed, cancellation requested). Source: `listReservationPage` in `lib/server/reservations.ts`, `$lib/reservation-views.ts`.

<script lang="ts">
	...
	let roomTypeId = $state(r.roomTypeId);
	const roomTypeLabel = $derived(
		data.roomTypeOptions.find((rt) => rt.id === roomTypeId)?.name ?? 'Select a room type'
	);
			...
			<div class="grid gap-4 lg:grid-cols-2">
				<div class="rounded-xl border border-border bg-surface p-4 shadow-sm">
					...
					<div class="mt-3 grid grid-cols-2 gap-3">
						...
						<div>
							<Label for="roomTypeId">Room category / type</Label>
							<Select.Root
								type="single"
								name="roomTypeId"
								bind:value={roomTypeId}
								disabled={data.roomIsCheckedIn}
							>
								<Select.Trigger id="roomTypeId" class="mt-1 w-full">
									{roomTypeLabel}
								</Select.Trigger>
								<Select.Content>
									{#each data.roomTypeOptions as rt (rt.id)}
										<Select.Item value={rt.id} label={rt.name} />
									{/each}
								</Select.Content>
							</Select.Root>
							{#if data.roomIsCheckedIn}
								<p class="mt-1 text-xs text-danger">
									A guest is currently checked in — check them out before changing the room type.
								</p>
							{:else if t}
								<a
							...
							{#if primaryPlan}
								<dl class="mt-3 space-y-2 text-sm">
									...
									<div class="flex justify-between gap-2">
										...
										<dd class="text-ink">
											{primaryPlan.securityDepositAmountCentavos != null
												? peso(primaryPlan.securityDepositAmountCentavos)
												: '—'}
										</dd>
									</div>
								</dl>
				...
				<div class="mt-3 space-y-3">
					<div>
						...
						<p class="mt-1 pl-6 text-xs text-ink-muted">
							...
							etc.) without deleting its booking history. This works alongside, not instead of, the <span
								class="font-medium">Operational Status</span
							>
							above — a room only shows up in guest availability searches when it's both
					...
					<div>
						...
						<p class="mt-1 pl-6 text-xs text-ink-muted">
							Informational only — flags this as a physically adjoining room so front desk can offer
							it alongside its pair to families or groups who want to book two connecting rooms. It
							doesn't link the two rooms in the system or affect search/pricing; front desk still
							books each room separately and pairs them manually.
						</p>
					</div>
					<div>
						...
						/>
						<p class="mt-1 text-xs text-ink-muted">
							Lower numbers list first — within the same floor on the front desk's room grid, and in
							this settings list. Rooms with the same sort order fall back to room number.
						</p>
					</div>
				</div>
			</div>

...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	const depositPolicies = await db
		.select({ id: securityDepositPolicies.id, amountCentavos: securityDepositPolicies.amountCentavos })
		.from(securityDepositPolicies)
		.where(eq(securityDepositPolicies.hotelId, hotelId));
	const depositAmountById = new Map(depositPolicies.map((p) => [p.id, p.amountCentavos]));
	const plans = planRows.map((p) => ({
		...p,
		securityDepositAmountCentavos: p.securityDepositPolicyId
			? depositAmountById.get(p.securityDepositPolicyId) ?? null
			: null
	}));
	...
	const roomTypeOptions = await db
	...
	// A currently checked-in guest is physically in this room right now — changing
	// its type would mismatch the front-desk grid's badge/color against a real,
	// present occupant, even though the guest's own booking record (`booking_rooms`)
	// keeps its own separate, already-correct room-type snapshot either way.
	const [occupied] = await db
		...
		.where(and(eq(roomAssignments.roomId, room.id), eq(bookings.status, 'checked_in')))
		.limit(1);
...
export const actions: Actions = {
	update: async (event) => {
		...
		const [current] = await db
			...
			.limit(1);
		if (!current) error(404, 'Room not found');

		if (current.roomTypeId !== parsed.data.roomTypeId) {
			const [occupied] = await db
				.select({ id: roomAssignments.id })
				.from(roomAssignments)
				.innerJoin(bookingRooms, eq(bookingRooms.id, roomAssignments.bookingRoomId))
				.innerJoin(bookings, eq(bookings.id, bookingRooms.bookingId))
				.where(and(eq(roomAssignments.roomId, roomId), eq(bookings.status, 'checked_in')))
				.limit(1);
			if (occupied) {
				return fail(400, {
					error:
						'This room is currently occupied — check out the guest before changing its room type.'
				});
			}
		}
	...
	delete: async (event) => {
		...
		redirect(303, `/${event.params.hotel}/management/settings/rooms`);
	}
};

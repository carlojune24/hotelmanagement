<script lang="ts">
	...
	let depositPolicyId = $state('');
	$effect(() => {
		depositPolicyId = p.securityDepositPolicyId ?? '';
	});
	let inclusions = $state<string[]>([]);
	$effect(() => {
		...
		<section class="rounded-xl border border-border bg-surface-2 p-5 shadow-sm">
			...
			<form method="POST" action="?/update" use:enhance class="mt-3 space-y-3">
				...
				<div>
					...
					<Select.Root type="single" name="securityDepositPolicyId" bind:value={depositPolicyId}>
						<Select.Trigger id="securityDepositPolicyId" class="mt-1 w-full">
							{data.securityDepositPolicies.find((c) => c.id === depositPolicyId)?.name ?? 'None'}
						</Select.Trigger>
						<Select.Content>
							{#each data.securityDepositPolicies as c (c.id)}
								<Select.Item value={c.id} label="{c.name} — ₱{peso(c.amountCentavos)}" />
							{/each}
						</Select.Content>
					</Select.Root>
				</div>

...
- [x] **Guest confirmation email (2026-09-09)** — diagram's "Booking Confirmation (email/SMS)". Email only for MVP; **SMS still post-MVP**.
  ...
  - **Not done:** attaching the BIR Invoice/OR PDF (that's BIR module Step 4); staff-facing email-log viewer / resend button; cancellation / payment-failed guest emails; real cross-client render QA (Litmus/EOM) — built to table+inline-style + VML-button best practice but not yet tested in Outlook/Gmail/Apple Mail for real.

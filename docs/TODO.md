...
- [x] **Guest confirmation email (2026-09-09)** — diagram's "Booking Confirmation (email/SMS)". Email only for MVP; **SMS still post-MVP**.
  ...
  - **Staff view + resend** (`(staff)/reservations/[kind]/[id]`): a "Confirmation email" card shows the order's `email_log` rows (status badge, recipient, timestamp, error) and a Resend / Send now action (`?/resendConfirmation`, `booking:write`, `sendBookingConfirmation(orderId, { force: true })` — `force` skips the idempotency guard). Staff shadcn/oklch operate-mode, matches the page's existing card + payment-badge idiom. `getRoomBookingDetail`/`getHallBookingDetail` now return `confirmationEmails`. Live-DB verified (guard skips, force logs a 2nd row, detail exposes both newest-first).

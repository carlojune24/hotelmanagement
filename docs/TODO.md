...
- [x] **Guest confirmation email (2026-09-09)** — diagram's "Booking Confirmation (email/SMS)". Email only for MVP; **SMS still post-MVP**.
  ...
  - **Next email types (not built):** **cancellation email** — email the guest when a *confirmed* booking is cancelled; depends on the staff-cancel-booking flow below (no transition path yet). A "needs follow-up" flag/view for bookings. An in-app preview of a sent email's body (body isn't stored — re-render from the order).
...
- [ ] **Guest email conversations (Mailgun) — planned** (added 2026-09-09, user wants a full two-way email thread per booking, inside the app). Moves off Gmail-as-relay onto Mailgun so guest **replies** are captured and threaded, not just left in a personal inbox. Built in layers so each ships value:
  ...
  - **C — Conversation model + staff UI.** Grow `email_log` into `guest_messages`: `direction` (`outbound`/`inbound`), `type` (`booking_confirmation` | `booking_cancelled` | `staff_reply` | `guest_reply` | …), `fromAddress`/`toAddress`, `subject`, `bodyText`, `bodyHtml`, `snippet`, `providerMessageId`, `inReplyTo`, `status` (`sent`/`failed`/`received`), `error`, `attachments` jsonb, `readAt`. Migrate existing rows as `outbound`. `alreadySent` still works. **UI:** the reservation-detail "Confirmation email" card becomes a **threaded transcript + reply composer** (`booking:write` to send a `staff_reply`, threaded with the right `In-Reply-To`); `(staff)/emails` becomes a real **inbox** — unread count + sidebar badge, "has guest reply" / "unmatched" filters, row opens the thread, manual "assign to booking" for unmatched. Rename the nav item "Emails" → "Messages"/"Inbox" at build time. Run `impeccable` (shape) on the thread + inbox — it's a meaningful new staff operate-mode surface.

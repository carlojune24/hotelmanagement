...
- [x] **Guest confirmation email (2026-09-09)** — diagram's "Booking Confirmation (email/SMS)". Email only for MVP; **SMS still post-MVP**.
  ...
  - **`email_log` table** (migration `0023`): one row per send attempt (`sent`/`failed`, `messageId`, `error`), FK to hotel + order. `alreadySent(orderId, type)` guards re-sends. Hook for a future staff resend UI.
